  ...
  if ('/netTokenNotification' === parsedUrl?.pathname) {
    ...
    if (!authHeader) {
      ...
      if (pathName && Constants.EXTENSION_SERVICE_END_POINTS.includes(pathName)) {
        ...
        route.sendResponse(res, Constants.HTTP_UNAUTHORIZED_STATUS_CODE, 'application/json', JSON.stringify({ message: "1"+CustomMessages.ERROR_MSG_MISSING_AUTHORIZATION_HEADER }));
      } else {
        route.sendResponse(res, Constants.HTTP_UNAUTHORIZED_STATUS_CODE, 'application/json', JSON.stringify({ message: "2"+CustomMessages.ERROR_MSG_MISSING_AUTHORIZATION_HEADER }));
      }
    } else {
      const pathName = parsedUrl?.pathname;
      if (authHeader && pathName && Constants.EXTENSION_SERVICE_END_POINTS.includes(pathName)) {
        ...
        console.log("endode"+encodedCredentials);
        const decrypt = paymentUtils.decryption(encodedCredentials);
        console.log("decrypt"+decrypt);
        decrypt && decrypt === process.env.PAYMENT_GATEWAY_EXTENSION_HEADER_VALUE ? handleRequest(req, res) : route.sendResponse(res, Constants.HTTP_UNAUTHORIZED_STATUS_CODE, 'application/json', JSON.stringify({ message: "3"+CustomMessages.ERROR_MSG_MISSING_AUTHORIZATION_HEADER }));
      } else {
        route.sendResponse(res, Constants.HTTP_UNAUTHORIZED_STATUS_CODE, 'application/json', JSON.stringify({ message: "4"+CustomMessages.ERROR_MSG_MISSING_AUTHORIZATION_HEADER }));
      }
    }
  }
}
